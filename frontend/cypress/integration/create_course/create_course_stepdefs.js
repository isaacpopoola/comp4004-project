...
And("Input course prereqs {string}", (prereqs) => {
    cy.wait(200);
    cy.get("input#prereqs.ant-input").type(prereqs);
});
...
And("Input registration deadline", () => {
    ...
    cy.get("input#create-course-courseregistrationdeadline").type(
        `${d.getFullYear()}-${d.getMonth()}-${d.getDate() + 1}{enter}`,
        {
            force: true,
        }
    );
...
And("Input drop deadline", () => {
    ...
    cy.get("input#create-course-coursedropdeadline").type(
        `${d.getFullYear()}-${d.getMonth()}-${d.getDate() + 1}{enter}`,
        {
            force: true,
        }
...
And("Input duration {float}", (duration) => {
    cy.wait(200);
    cy.get("input#create-course-courseduration.ant-input-number-input").type(
        duration
    );
});
...
And("Input student limit {int}", (limit) => {
    cy.wait(200);
    cy.get(
        "input#create-course-coursestudentlimit.ant-input-number-input"
    ).type(limit);
});

And("Input credits {float}", (credits) => {
    cy.wait(200);
    cy.get("input#create-course-coursecredits.ant-input-number-input").type(
        credits
    );
});
...
And("Input price {float}", (price) => {
    cy.wait(200);
    cy.get("input#create-course-courseprice.ant-input-number-input").type(
        price
    );
});
...
Then(`{string} should exist in table`, (course_code) => {
    ...
    cy.get(
        "#root > section > section > main > div > div:nth-child(2) > div > div > div > div > div > div > table > tbody"
    ).should("contain", course_code);
});

Then("Error should exist on course-code input", () => {
    cy.wait(200);
    cy.get(
        "#basic > div:nth-child(1) > div.ant-col.ant-form-item-control > div.ant-form-item-explain.ant-form-item-explain-error > div"
    ).should(($div) => {
        const text = $div.text();
...
Then("Error should exist on course name input", () => {
    cy.wait(200);
    cy.get(
        "#basic > div:nth-child(2) > div.ant-col.ant-form-item-control > div.ant-form-item-explain.ant-form-item-explain-error > div"
    ).should(($div) => {
        const text = $div.text();
...
Then("Error should exist on course description input", () => {
    cy.wait(200);
    cy.get(
        "#basic > div:nth-child(3) > div.ant-col.ant-form-item-control > div.ant-form-item-explain.ant-form-item-explain-error > div"
    ).should(($div) => {
        const text = $div.text();
