...
And("I delete course with course code {string}", (course_code) => {
    ...
    cy.get(
        "#root > section > section > main > div > div:nth-child(2) > div > div > div > div > div > div > table > tbody > tr > td:nth-child(2)"
    )
        .each(($e1, index, $list) => {
            const text = $e1.text();
            ...
            if (text.includes(course_code)) {
                exists.is = true;
                cy.get(
                    `#root > section > section > main > div > div:nth-child(2) > div > div > div > div > div > div > table > tbody > tr:nth-child(${
                        index + 1
                    }) > td:nth-child(9) > div > div > a`
                ).click();
            }
        })
        .then(() => {
...
Then(`{string} should not exist in table`, (course_code) => {
    ...
    cy.get(
        "#root > section > section > main > div > div:nth-child(2) > div > div > div > div > div > div > table > tbody > tr "
    ).each(($e1, index, $list) => {
        const text = $e1.text();
