...
router.post("", async (req, res) => {
    ...
    if (!student || !course) {
    ...
    else if (course.course_registration_deadline && today > course.course_registration_deadline) {
        ...
        let student_registration = { student_id: student.id, course_code: course.course_code};
        try {
            // register
            ...
            if (course_after.registered_students >= course_after.course_student_limit) {
                StudentRegisteredCourses.destroy({ where: { student_id: student.id, course_code: course_code } });
                Courses.update(
                ...
                return res.status(200).send({ message: "Student is now registered" });
            }
        }
        catch {
